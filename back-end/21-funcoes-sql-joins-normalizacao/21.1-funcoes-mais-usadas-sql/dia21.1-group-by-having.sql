...
--    FROM sakila.film
--    GROUP BY rating;
	SELECT rating, AVG(length) duração_média
    FROM sakila.film
    GROUP BY rating
...
--    FROM sakila.film
--    GROUP by rating;
    SELECT rating, SUM(replacement_cost) custo_total_substituição
    FROM sakila.film
    GROUP by rating
    HAVING custo_total_substituição > 3950.50
    ORDER BY custo_total_substituição;
